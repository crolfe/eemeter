...
def import_hpxml(filename):
    ...
    for info in consumption_infos:
        fuel_type = info.xpath(
            "ns2:ConsumptionType/ns2:Energy/ns2:FuelType",
            namespaces=ns)
        unit_of_measure = info.xpath(
            "ns2:ConsumptionType/ns2:Energy/ns2:UnitofMeasure",
            namespaces=ns)
        ...
        consumption_details = info.xpath(
            "ns2:ConsumptionDetail", namespaces=ns)
        records = []
        for details in consumption_details:
            ...
            records.append({"start": start, "end": end,
                            "estimated": estimated, "value": usage})
...
def import_green_button_xml(filename):
    ...
    def get_record(reading):
        ...
        duration_s = int(time_period.xpath(
            "*[local-name() = 'duration']")[0].text)
...
def import_seed_timeseries(db_url):
    ...
    for building_id, meter_ids in building_meters.items():
        ...
        for meter_id in meter_ids:
            ...
            timeseries = select(
                [seed_timeseries]).where(
                seed_timeseries.c.meter_id == meter_id)
            ...
            consumption_data = ConsumptionData(
                records, fuel_type, unit_name, record_type="arbitrary")
            consumption.append(consumption_data)
