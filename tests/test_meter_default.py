...
@pytest.fixture(params=[([1, 14.5, 1, 17.8, 8], [1, 14.5, 1], 6849.29, 615.69,
                         0, 0, 0, 0, 1, 1, "degC", 693.58, 877.00, 2587.61,
                         1805.00),
                        ...
                        ([0, 65, 2, 71, 3], [0, 65, 2], 4700.22, 3157.17, 0, 0,
                         0, 0, 1, 1, "degF", 1248.45, 1578.58, 4657.70,
                         3249.00),
                        ])
def default_residential_outputs_1(request,
    ...
    elec_model = AverageDailyTemperatureSensitivityModel(
    ...
    elec_gen = MonthlyBillingConsumptionGenerator(
        "electricity", "kWh", temp_unit, elec_model, elec_params)
    elec_consumption_data = elec_gen.generate(
    ...
    gas_gen = MonthlyBillingConsumptionGenerator(
        "natural_gas", "therm", temp_unit, gas_model, gas_params)
    gas_consumption_data = gas_gen.generate(
        gsod_722880_2012_2014_weather_source, datetimes)
...
def test_default_residential_meter(default_residential_outputs_1,
    ...
    baseline_period = Period(datetime(2012, 1, 1, tzinfo=pytz.utc),
                             retrofit_start_date)
    reporting_period = Period(retrofit_completion_date,
                              datetime(2014, 12, 31, tzinfo=pytz.utc))
    project = Project(location, [elec_consumption_data, gas_consumption_data],
    ...
    assert_allclose(
        result.get_data('annualized_usage',
                        tags=['electricity', 'baseline']).value,
        elec_annualized_usage,
        rtol=RTOL,
        atol=ATOL)
...
def test_default_residential_meter_bad_inputs():
    ...
    with pytest.raises(ValueError):
        DefaultResidentialMeter(
            settings={
                "heating_balance_temp_high": 0,
                "heating_balance_temp_x0": 1,
                "heating_balance_temp_low": 2})

    with pytest.raises(ValueError):
        DefaultResidentialMeter(
            settings={
                "cooling_balance_temp_high": 0,
                "cooling_balance_temp_x0": 1,
                "cooling_balance_temp_low": 2})

    with pytest.raises(ValueError):
        DefaultResidentialMeter(
            settings={
                "electricity_heating_slope_high": -1})

    with pytest.raises(ValueError):
        DefaultResidentialMeter(
            settings={
                "natural_gas_heating_slope_high": -1})

    with pytest.raises(ValueError):
        DefaultResidentialMeter(
            settings={
                "electricity_cooling_slope_high": -1})
