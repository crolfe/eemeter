...
def test_gsod_weather_source(periods, gsod_weather_source):
    ...
    assert isinstance(json_data["records"][0]["datetime"], str)
...
def test_isd_weather_source(periods, isd_weather_source):
    ...
    assert_allclose(
        hourly_temps[0][
            :5], [
            69.98, 66.92, 64.04, 62.96, 62.96], rtol=RTOL, atol=ATOL)
    ...
    assert_allclose(
        daily_temps[
            :3], [
            66.466, 66.098, 66.685], rtol=RTOL, atol=ATOL)
...
def test_tmy3_weather_source(periods, tmy3_weather_source):
    ...
    assert_allclose(
        avg_temps, [
            68.1822, 73.05548, 74.315], rtol=RTOL, atol=ATOL)
