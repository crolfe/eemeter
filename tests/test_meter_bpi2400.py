...
@pytest.fixture(params=[([10, 58, 2, 72, 1], [10, 65, 1], 1248.4575, 1578.5882,
                         0, 0,
                         [15, 15, 7, 7, 11, 11, 16, 16], 1080, 4657.6997,
                         3249.0001, "degF"),
                        ([10, 14.4, 2, 22.22, 1], [10, 18.33, 1], 693.5875,
                         876.9934, 0, 0,
                         [15, 15, 7, 7, 11, 11, 16, 16], 1080, 2587.610,
                         1805.0001, "degC")])
def bpi_2400_1(request, gsod_722880_2012_2014_weather_source):
    ...
    gen_elec = MonthlyBillingConsumptionGenerator(
        "electricity", "kWh", temp_unit, elec_model, elec_params)
    gen_gas = MonthlyBillingConsumptionGenerator(
        "natural_gas", "therm", temp_unit, gas_model, gas_params)
    elec_consumptions = gen_elec.generate(
        gsod_722880_2012_2014_weather_source, datetimes)
    gas_consumptions = gen_gas.generate(
        gsod_722880_2012_2014_weather_source, datetimes)

    average_daily_usages_elec = elec_consumptions.average_daily_consumptions()[
        0]
...
def test_bpi2400(bpi_2400_1,
    ...
    for result, params, average_daily_usages, cvrmse in [elec, gas]:
        ...
        assert_allclose(result_elec.get_data('hdd_tmy').value, normal_hdd,
                        rtol=RTOL, atol=ATOL)
        assert_allclose(
            result.get_data("temp_sensitivity_params_bpi2400").value.to_list(),
            params,
            rtol=RTOL,
            atol=ATOL)
        ...
        assert_allclose(
            result.get_data("estimated_average_daily_usages_bpi2400").value,
    ...
    for result in [result_elec, result_gas]:
        ...
        assert result.get_data(
            'has_enough_periods_with_high_cdd_per_day').value
        assert result.get_data(
            'has_enough_periods_with_high_hdd_per_day').value
        ...
        assert result.get_data(
            'meets_model_calibration_utility_bill_criteria').value
